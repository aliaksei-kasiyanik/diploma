...
public class SteadyingProcessWithOpressionAndClarification extends SteadyingProcessWithClarification {
    ...
    public SteadyingProcessWithOpressionAndClarification(AuxRungeKuttaMethod auxMethod, double EPS, double w, SteadyingEquation steadyingEquation, double[] opressorCoefficients) {
...
//        double newW = - 1.0 / 1000.0;
        ...
        if (auxMethod.getSigma() % 2 == 0) { //четное сигма
            for (int m = 0; m < n; m++) {
                ...
                double[] wr = scalarMultipy(w, r_with_opressor(Y));
